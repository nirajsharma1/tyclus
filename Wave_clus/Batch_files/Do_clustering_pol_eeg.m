function Do_clustering_pol_eeg(file_to_cluster)

print2file = 1;                             %for saving printouts.

handles.par.w_pre=25;                       %number of pre-event data points stored (default 20)
handles.par.w_post=50;                      %number of post-event data points stored (default 44)
...
handles.par.inputs = 64;                    %number of inputs to the clustering (default 10)
handles.par.scales = 8;                     %scales for wavelet decomposition (default 4)
...
handles.par.SWCycles = 8000;                 %number of montecarlo iterations (default 100)
...
handles.par.min_clus_abs = 30;              %minimum cluster size (absolute value) (default 60) 
...
sr = 250; % sampling frequency in Hz
min_ref_per=100;                                    %minimum refractory period (in ms)
ref = floor(min_ref_per*sr/1000);                  %number of counts corresponding the minimum refractory period
handles.par.sr = sr; 
handles.par.ref = ref;

%parameter for polytrodes: time difference between channels
handles.par.include_time = 'n';
...
%     file_to_cluster = strcat('polytrode',num2str(polytrode),'_spikes.mat');
    ...
    inspk = wave_features_pol(spikes,handles);              %takes wavelet coefficients.
