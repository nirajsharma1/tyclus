...
% function f_extract_spikes_eeg_NS_v6(data_filename)

patient_name = 'JR';

data_dir = (['/Users/nsharma/Desktop/PhD/data_processed/' patient_name '/all/data/']);

cd([data_dir]);

data_filename = (['filt_eeg_data_' patient_name]);

% markers_dir = (['/Users/nsharma/Desktop/PhD/validation/' patient_name '/data_forCP/']);

% cd([markers_dir]);

markers_filename = ([patient_name '_realigned_GFP_12ms.dat']);
...
marker1 = strmatch('B', marker_description);
...
spike_sample = double(marker_position(spike_elements));
spike_chann = double(marker_chann(spike_elements));
...
out_data_file = [directory_save_pathway 'filt_eeg_spikes_' patient_name];
...
markers_datafile = [directory_save_pathway 'spikes_markers_' patient_name];
...
details_file = [directory_save_pathway 'spike_details_' patient_name];
